...
namespace OpenCVForUnity.UnityIntegration
{
    ...
    public static partial class OpenCVMatUtils
    {
        ...
        private static ComputeShader _renderTextureToMatComputeShader;
        private static ComputeShader _matToRenderTextureComputeShader;
        ...
        private static void DispatchMatToRenderTextureComputeShader(GraphicsBuffer graphicsBuffer, RenderTexture renderTexture, bool flip, int flipCode)
        {
            ...
            if (_matToRenderTextureComputeShader == null)
                _matToRenderTextureComputeShader = Resources.Load<ComputeShader>("MatToRenderTexture");

            int kernelIndex = GetKernelIndexForMatToRenderTextureComputeShader(graphicsBuffer, renderTexture);

            SetupMatToRenderTextureComputeShader(_matToRenderTextureComputeShader, graphicsBuffer, renderTexture, flip, flipCode, kernelIndex);

            // Execute the shader
            _matToRenderTextureComputeShader.Dispatch(kernelIndex, renderTexture.width / 8, renderTexture.height / 8, 1);
        }
        ...
        {
            // Load the shader only if it has not been loaded yet
            if (_renderTextureToMatComputeShader == null)
                _renderTextureToMatComputeShader = Resources.Load<ComputeShader>("RenderTextureToMat");

            int kernelIndex = GetKernelIndexForRenderTextureToMatComputeShader(renderTexture, graphicsBuffer);

            SetupRenderTextureToMatComputeShader(_renderTextureToMatComputeShader, renderTexture, graphicsBuffer, flip, flipCode, kernelIndex);

            // Dispatch the shader
            _renderTextureToMatComputeShader.Dispatch(kernelIndex, renderTexture.width / 8, renderTexture.height / 8, 1);
        }
...
#else
                            Array.Copy(graphicsBufferDataNativeArray.ToArray(), graphicsBufferData, graphicsBufferDataNativeArray.Length);
        ...
        private static async Awaitable GetDataContinuousAsync<T>(GraphicsBuffer graphicsBuffer, Mat mat, CancellationToken cancellationToken) where T : unmanaged
        {
            ...
            {
                OpenCVMatUtils.CopyToMat<T>(graphicsBufferDataNativeArray, mat);
            }
            else
            {
#if NET_STANDARD_2_1
